      ...
      <div style="margin-top:10px">
        <el-table border :data="lanjuData" style="width: 100%" class="table_1">
          ...
          <el-table-column align="center" label="操作" width="100px">
            <template slot-scope="scope">
              <el-button v-if="scope.row.STATUS == 8||scope.row.STATUS == 5" @click="_EditDetail(scope.row)" type="primary"
                size="mini" icon="el-icon-edit" circle></el-button>
              <el-button v-else @click="_CheckDetail(scope.row)" type="warning" size="mini" icon="el-icon-search" circle>
              </el-button>
            </template>
          </el-table-column>
        </el-table>
        ...
        <table width="100%" border="0px" cellspacing="0px" cellpadding="0">
          ...
          <tr>
            ...
            <td v-if="submitForm.STATUS==8" colspan="3" rowspan="1" style="height:30px">
              <el-date-picker type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="预计出图日期"
                v-model="submitForm.EXPECTED_DRAW_DATE" style="width:60%;"></el-date-picker>
            </td>
            <td v-if="submitForm.STATUS==5" colspan="3" rowspan="1" style="height:30px">
              {{ submitForm.EXPECTED_DRAW_DATE | datatrans }}</td>
            <td class="grayTD" colspan="2" rowspan="1" style="height:30px">设计图附件</td>
            <td colspan="2" rowspan="1" style="height:30px" v-if="submitForm.STATUS==5">
              <div>
                <el-upload class="upload-de" :action="Global.baseUrl + '/Lanju/UploadFiles'" drag multiple
                  :on-change="handleChange" :on-remove="handleRemove" :on-success="handleSuccess" :on-error="handleError"
                  ref="upload" :auto-upload="false" :file-list="this.fileListGM" :data="{ CID: CID, dateStamp: dateStamp }">
                  <div style="text-align:center">
                    <i class="el-icon-upload2" style="height:30px;margin-top:10px;">
                      <span>上传附件</span>
                    </i>
                  </div>
                </el-upload>
...
export default {
  ...
  methods: {
    ...
    _editSubmit(val) {
      ...
      //判断是否填完所有信息
      if (this.submitForm.EXPECTED_DRAW_DATE == "" && val != 0 && val != 4) {
