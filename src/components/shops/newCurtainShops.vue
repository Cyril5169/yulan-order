...
export default {
  ...
  methods: {
    ...
    querySearchAsync(queryString, cb) {
      GetAsyncItemData({ condition: this.searchKey.toUpperCase() }, { loading: false }).then(res => {
        var results = [];
    ...
    searchCurtain() {
      ...
      GetCurtainTemplateAndModel({ item_no: this.searchKey }).then((res) => {
        ...
        if (this.templateData.length) {
          this.noData = false;
          //帘款
          var headData = this.templateData.filter(item => item.NC_PART_TYPECODE == "LK");
          if (headData.length) {
            this.curtainHeadData = headData[0];
            //是否标定
            this.isStandard = this.curtainHeadData.NCT_STANDARD != 'N';
            //宽高，活动，位置，套数
          ...
          this.previewUrlList.push({
            type: 'LK',
            src: `http://www.luxlano.com/ddkc/ckfinder/userfiles/images/pimg/${this.curtainHeadData.ITEM_NO}.jpg`,
            show: true
          })
          //拼接出窗帘
          var curtain_temp = [];
    ...
    handleMesutieCommand(common, index) {
      var oneCurtain = this.curtainData[index];
      oneCurtain.NCM_MESUTIE = common;
    },
    //改变打开方式
    handleKaikouCommand(common, index) {
      var oneCurtain = this.curtainData[index];
      oneCurtain.NCM_KAIKOU = common;
    },
    //改变工艺方式
    ...
    handleBianCommand(common, index) {
      var oneCurtain = this.curtainData[index];
      if (oneCurtain.NC_PART_TYPECODE == 'LS') {
        if (common == "4B" && oneCurtain.NCM_BIAN != "4B") {
          //显示拉边条
          //先看看当前数据有没有这个拉边条，有的话应该是bug
          var lbtItemNow = this.curtainData.filter(item => item.NCM_PID == oneCurtain.NC_MODEL_ID && item.NC_PART_TYPECODE == "LBT");
          ...
          //去掉拉边条
          //找到有没有拉边条
          var lbtItem = this.curtainData.filter(item => item.NCM_PID == oneCurtain.NC_MODEL_ID && item.NC_PART_TYPECODE == "LBT");
          //应该只有一个拉边条，但是循环一下，保险
    ...
    onCheckChange(checked, row) {
      ...
      if (fatherCurtain.length) {
        ...
        if (checked) {
          ...
          //勾选上的时候看需不需要加载自己的拉边条，看自己是不是需要加载的那个
          //从父节点看是否需要加载
    ...
    beforeAddCar() {
      ...
      //拉边条的判断
      var lsItem = this.chooseCurtainData.filter(item => item.BIAN_ENABLE > 0 && item.NCM_BIAN == "4B");
      if (lsItem.length) {
      ...
      for (var i = 0; i < curtains.length; i++) {
        ...
        if (oneCurtain.NC_PART_TYPECODE == 'LT') {
          ...
          for (var j = 0; j < childrenCurtain.length; j++) {
            ...
            if (height > 2.8 && this.curtainHeadData.FIX_TYPE == '02') {
