  ...
  <el-card shadow="never">
    ...
    <el-dialog width="65%" v-if="dialogTableVisible" :visible.sync="dialogTableVisible" :close-on-click-modal="false">
      ...
      <el-input clearable v-if="curtainData[chooseIndex].productType !== 'GY'" placeholder="输入商品型号查找"
        style="width:250px;margin: 0 0 10px 10px;" v-model.trim="searchKey" @clear="initSearch" @keyup.enter.native="initSearch">
...
export default {
  ...
  methods: {
    ...
    getCurtainMsg(data) {
      ...
      //面料提示
      this.getMLTip(this.curtainData);
    },
    //获取面料的提示
    getMLTip(originData) {
      ...
      for (var i = 0; i < originData.length; i++) {
        ...
        if (itemNoList.indexOf(oneCurtain.itemNo) == -1)
            ...
            for (var i = 0; i < res.data.length; i++) {
              ...
              if (data.length) {
                for (var j = 0; j < data.length; j++) {
                  if (data[j].remark.indexOf(res.data[i].NOTE_NOTE) == -1) {
                    if (res.data[i].NOTE_NOTE) {
                      data[j].remark += res.data[i].NOTE_NOTE;
                    }
                    if (res.data[i].NOTE) {
    ...
    chooseItemNo() {
      ...
      this.curtainData = this.getStoreData(this.curtainData);
      //面料提示
      this.getMLTip([this.curtainData[this.chooseIndex]]);
        ...
        Axios.post("http://ljsp.ubxiu.com:8098/api/getXXDMX", postData, {
          ...
          if (res.data && res.data.data) {
            ...
            GetUnImportOrderByItem({ itemNo: itemNo }, { loading: false }).then(res2 => {
              ...
              var data = originData.filter((item) => item.itemNo == itemNo);
              if (data.length) {
                for (var j = 0; j < data.length; j++) {
